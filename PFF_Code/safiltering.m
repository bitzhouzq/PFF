function filtered_img = safiltering(ifshow, img, ss, sr, niter, se, div)
if (~exist('ss','var'))
   ss = 3;
end
if (~exist('sr','var'))
...
if ifshow
    subplot(floor(sqrt(niter)), floor(sqrt(niter))+1, 1);
    imshow(L); drawnow;
    title('raw');
end
...
for ii = 1:niter
    ...
    if ifshow
        subplot(floor(sqrt(niter)), floor(sqrt(niter))+1, ii+1);
        imshow(L); drawnow;
        title(strcat('safiltering itertion ', num2str(ii)));
    end
%     figure(12), imshow(L); drawnow;
end
% figure(12), imshow(L);
filtered_img = gather(L);
end